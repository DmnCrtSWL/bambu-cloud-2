<template>
  <AdminLayout>
    <div class="max-w-4xl mx-auto rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
      <h3 class="font-semibold text-lg text-gray-800 dark:text-white/90 mb-6">Editar Usuario</h3>
      <form @submit.prevent="handleSubmit">
        <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
          
          <!-- Nombre Completo (12 cols) -->
          ...
          <!-- Helper logic for password toggle -->
...
const fetchUser = async () => {
    try {
        const response = await fetch(`${import.meta.env.VITE_API_URL || '${import.meta.env.VITE_API_URL || 'http://localhost:3001'}'}/api/users/${userId}`);
        if (response.ok) {
...
const handleSubmit = async () => {
    // Validation
    if (!formData.name || !formData.username || !formData.role || !formData.email || !formData.pin) {
