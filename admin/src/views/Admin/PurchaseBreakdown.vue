...
const fetchPurchase = async () => {
    try {
        const response = await fetch(`${import.meta.env.VITE_API_URL || '${import.meta.env.VITE_API_URL || 'http://localhost:3001'}'}/api/purchases/${purchaseId}`);
        if(response.ok) {
            ...
            if(purchase.value.status === 'Desglosado') {
               ...
               try {
                   const itemsResponse = await fetch(`${import.meta.env.VITE_API_URL || '${import.meta.env.VITE_API_URL || 'http://localhost:3001'}'}/api/purchases/${purchaseId}/items`);
                   if (itemsResponse.ok) {
...
const saveBreakdown = async () => {
    ...
    try {
        const response = await fetch(`${import.meta.env.VITE_API_URL || '${import.meta.env.VITE_API_URL || 'http://localhost:3001'}'}/api/purchases/${purchaseId}/items`, {
            method: 'POST',
            headers: {
