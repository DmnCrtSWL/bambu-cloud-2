    ...
    try {
        ...
        if(response.ok) {
            ...
            if(purchase.value.status === 'Desglosado') {
               ...
               try {
                   const itemsResponse = await fetch(`http://localhost:3001/api/purchases/${purchaseId}/items`);
                   if (itemsResponse.ok) {
...
const saveBreakdown = async () => {
    ...
    try {
        const response = await fetch(`http://localhost:3001/api/purchases/${purchaseId}/items`, {
            method: 'POST',
            headers: {
