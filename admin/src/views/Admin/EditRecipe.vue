<template>
  <AdminLayout>
    <div class="max-w-4xl mx-auto rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
      <h3 class="font-semibold text-lg text-gray-800 dark:text-white/90 mb-6">Editar Receta</h3>
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
            ...
            <!-- Actions -->
             <div class="col-span-12 flex justify-end gap-3 mt-4 border-t border-gray-100 pt-6 dark:border-gray-700">
                <Button size="md" variant="outline" @click="$router.push('/recipes')">Cancelar</Button>
                <Button size="md" variant="primary" type="submit">Guardar Cambios</Button>
             </div>
...
const activeVariantTotalCost = computed(() => {
    return activeVariant.value?.items.reduce((sum: number, item: any) => sum + item.totalCost, 0) || 0;
});
...
const handleProductInput = async () => {
    ...
    try {
        const response = await fetch(`${import.meta.env.VITE_API_URL || '${import.meta.env.VITE_API_URL || 'http://localhost:3001'}'}/api/products/search?q=${encodeURIComponent(newItem.productName)}`);
        if (response.ok) {
...
const selectProduct = async (name: string) => {
    ...
    showSuggestions.value = false;
    try {
        const response = await fetch(`${import.meta.env.VITE_API_URL || '${import.meta.env.VITE_API_URL || 'http://localhost:3001'}'}/api/products/cost?name=${encodeURIComponent(name)}`);
        if(response.ok) {
...
// Fetch existing data & Sync
const syncLatestCosts = async () => {
    let updatedCount = 0;
    ...
    for (const variant of variants.value) {
        ...
        for (const item of variant.items) {
            ...
            try {
                const response = await fetch(`${import.meta.env.VITE_API_URL || '${import.meta.env.VITE_API_URL || 'http://localhost:3001'}'}/api/products/cost?name=${encodeURIComponent(item.productName)}`);
                if (response.ok) {
                    const data = await response.json();
                    const latestCost = Number(data.cost);
                    const currentCost = Number(item.unitCost);
                    
                    if (Math.abs(latestCost - currentCost) > 0.01) {
                        item.unitCost = latestCost;
                        item.unit = data.unit || item.unit; 
                        item.totalCost = item.quantity * latestCost;
                        updatedCount++;
                    }
                }
            } catch (e) {
                console.error("Error syncing cost for " + item.productName, e);
            }
        }
    }
    // Removed log
...
onMounted(async () => {
    try {
        const response = await fetch(`${import.meta.env.VITE_API_URL || '${import.meta.env.VITE_API_URL || 'http://localhost:3001'}'}/api/recipes/${recipeId}`);
        if(response.ok) {
            const data = await response.json();
            formData.name = data.name;
            formData.category = data.category;
            
            // Map variants from DB response
            if (data.variants && data.variants.length > 0) {
                 variants.value = data.variants.map((v: any) => ({
                    id: v.id,
                    name: v.name,
                    items: v.items ? v.items.map((i:any) => ({
                ...
                // Fallback if no variants (should not happen with migration)
                variants.value = [{ name: 'Estándar', items: [] }];
            }

            // Sync Real Time Costs
            await syncLatestCosts();
...
const handleSubmit = async () => {
    if (!formData.name) {
        alert('Completa el nombre y agrega al menos un insumo');
        return;
    }
    
    // Validate at least one item in generic
