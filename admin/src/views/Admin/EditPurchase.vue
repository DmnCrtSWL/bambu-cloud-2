<template>
  <AdminLayout>
    <div class="max-w-4xl mx-auto rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
      <h3 class="font-semibold text-lg text-gray-800 dark:text-white/90 mb-6">Editar Ticket</h3>
...
const fetchPurchase = async () => {
    try {
        ...
        if(response.ok) {
           ...
           formData.ticketNumber = data.ticketNumber;
...
const handleSubmit = async () => {
    // Validation
    if (!formData.ticketNumber || !formData.provider || !formData.date || !formData.total || !formData.paymentMethod) {
