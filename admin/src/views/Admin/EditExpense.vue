<template>
  <AdminLayout>
    <div class="max-w-4xl mx-auto rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
      <h3 class="font-semibold text-lg text-gray-800 dark:text-white/90 mb-6">Editar Gasto</h3>
...
const fetchExpense = async () => {
    try {
        const response = await fetch(`${import.meta.env.VITE_API_URL || '${import.meta.env.VITE_API_URL || 'http://localhost:3001'}'}/api/expenses/${expenseId}`);
        if(response.ok) {
           const data = await response.json();
           formData.concept = data.concept;
           formData.paidTo = data.paidTo;
