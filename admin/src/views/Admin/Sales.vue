        ...
        <div class="max-w-full overflow-x-auto custom-scrollbar">
          ...
          <table v-if="viewMode === 'clients'" class="min-w-full">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
               <tr v-if="loading">
                  <td colspan="8" class="px-5 py-8 text-center text-gray-500 text-sm">Cargando ventas...</td>
               </tr>
              <tr v-else-if="clientTableData.length === 0">
                <td colspan="8" class="px-5 py-8 text-center text-gray-500 text-sm">No hay ventas registradas en este periodo</td>
              </tr>
              <tr v-for="order in clientTableData" :key="order.id" class="border-t border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                ...
                <td class="px-5 py-4 sm:px-6">
                   <p class="text-gray-500 text-theme-sm dark:text-gray-400 font-medium">
                       {{ order.customerName || 'Venta en Barra' }}
                   </p>
                </td>
                <td class="px-5 py-4 sm:px-6">
                   <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ formatDate(order.created_at) }}</p>
                </td>
                <td class="px-5 py-4 sm:px-6">
                ...
                <td class="px-5 py-4 sm:px-6">
                   <p class="text-brand-600 font-bold text-theme-sm dark:text-brand-400">${{ order.total.toFixed(2) }}</p>
                </td>
                <td class="px-5 py-4 sm:px-6">
...
const rangeStart = ref(new Date().toLocaleDateString('en-CA'));
const rangeEnd = ref(new Date().toLocaleDateString('en-CA'));
...
const fetchSales = async () => {
  ...
  try {
    ...
    const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/orders?${params.toString()}`);
    if (response.ok) {
