...
const fetchReport = async () => {
    ...
    try {
        ...
        const res = await fetch(`${import.meta.env.VITE_API_URL || '${import.meta.env.VITE_API_URL || 'http://localhost:3001'}'}/api/reports/daily-sales?date=${selectedDate.value}&userId=${userIdForReport}`);
        if (res.ok) {
