            ...
            <div class="p-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/50">
                <!-- Custom Sale -->
                 <button 
                    ...
                    class="w-full h-12 flex justify-center items-center bg-white text-brand-600 font-bold rounded-xl active:bg-gray-50 active:scale-95 transition-all text-base border border-brand-200 shadow-sm"
                 >
                    + Venta Personalizada
                 </button>
            </div>
...
const handleCustomerSearch = async () => {
    ...
    try {
        const res = await fetch(`http://localhost:3001/api/customers?q=${encodeURIComponent(query)}`);
        if (res.ok) {
...
const processPayment = async (method) => {
    ...
    try {
        ...
        // SCENARIO 1: Existing Single Order (Loaded from Kanban) -> UPDATE it
            ...
            res = await fetch(`http://localhost:3001/api/orders/${currentOrderId.value}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    paymentMethod: method,
