<template>
  <div class="relative" ref="dropdownRef">
    ...
    <div
      v-if="dropdownOpen"
      class="absolute -right-[240px] mt-[17px] flex h-auto max-h-[500px] w-[350px] flex-col rounded-2xl border border-gray-200 bg-white p-3 shadow-theme-lg dark:border-gray-800 dark:bg-gray-dark sm:w-[361px] lg:right-0"
    >
      <div
      ...
      <ul class="flex flex-col h-auto overflow-y-auto custom-scrollbar">
        <li v-if="notifications.length === 0" class="p-4 text-center text-gray-500 text-sm">
          ...
          >
            ...
            <span class="block">
              <span class="mb-1.5 block text-theme-sm text-gray-500 dark:text-gray-400">
                ...
                <span class="block text-xs mt-1">{{ notification.message }}</span>
              </span>
...
// Format logic moved to composable ideally, or handled here if purely display
// We are expecting notification objects to have { userName, userImage, action, project, time }
// Our composable emits objects with slightly different structure? 
// Composable emits: { id, type, userImage, userName, message, time, read }
// Template expects: notification.userName, .action, .project, .type
// We should update the TEMPLATE to support our new structure or Map it.

// Let's UPDATE THE TEMPLATE to match our simpler structure:
// User Image -> userImage
// Line 1: userName + message (action/project replaced by message)
// Line 2: type + time
